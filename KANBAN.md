...
**남은 관찰/부채 (후속) 추가**
- **자살골 잔여 비율(~19%, KB-62/63으로 대폭 개선)** — KB-61(접선 오버슈트)만으로는 몸통충돌 디플렉션이 남아 자책골 비율이 그대로였음(측정 방법 교훈: 득점 시점 "공 반경 0.55m 이내 로봇"이라는 근접 휴리스틱은 실제 rapier 충돌 이벤트와 비교 검증한 결과 이 시나리오에선 우연히 정확했다 — 하지만 보정 로직 자체를 근접 기준으로 게이팅한 건 진짜 버그였음, 상세는 Done 항목). KB-62(자기 골 근처 감속)+KB-63(마지막 접촉자 기준 디플렉션 보정)으로 30분 헤드리스 재측정 결과 자책골 26골 중 5골(19%)로 대폭 감소(총 득점 빈도도 절반 이하로 감소 — 수비가 실제로 걷어내기 시작함). 완전히 0은 아니라 추가 여지는 있으나(예: 태클성 몸싸움 중 디플렉션, 벽 반사 후 자책 등) 우선순위 낮음 판단, 필요 시 재조사.
...
## Done
**자살골 몸통충돌 디플렉션 완화 ✅** (branch `feat/interactivity`)
- [x] KB-63 자책골 디플렉션 보정(마지막 접촉자 기준) — 물리 레이어에 `last_ball_toucher`(rapier 충돌 이벤트 기반)를 추적, 그 로봇의 자기 골 1.5m 이내에서 공 속도가 자기 골 쪽 성분을 가지면 그 성분만 제거(`correct_own_goal_deflections`). **1차 설계(로봇-공 "현재 근접" 게이팅)는 재측정에서 효과가 없었음** — 실측 재현 결과, 디플렉션은 로봇과 공이 떨어진 뒤에도 공이 여세로 계속 굴러 골까지 들어가는 패턴이 대부분이라 근접 게이팅으로는 정작 필요한 순간을 놓쳤음. "현재 근접"이 아니라 "마지막 접촉자"(충돌 이벤트로 기록, 근접 불필요) 기준으로 재설계해 해결. (테스트: 재현 수치 그대로 쓴 유닛테스트 3개 RED→GREEN, cargo test 110/110). 실측(정확한 충돌 이벤트 기반 지표로 재검증): 30분 헤드리스 자책골 26골 중 5골(19%, KB-61 이후 58%에서 대폭 개선).
- [x] KB-62 자기 골 근처 접근 감속 — `own_goal_caution_scale`: 자기 골 1.5m 이내에서 공에 접근할 때 추력을 1.0→0.35까지 선형으로 낮춤(공격형/수비형 공용). 단독으로는 실측 개선이 없었으나(로봇이 아니라 공 자체의 속도가 원인인 경우가 대부분) KB-63과 결합해 유지.

**자살골 조사 + 부분 수정 ✅** (branch `feat/interactivity`)
- [x] KB-61 방어형 목표 접선 외삽 오버슈트 수정 — `guard_target`의 LOOKAHEAD 외삽이 공 속도의 접선 성분이 크면(골라인과 나란히 빠르게 흐름) 목표를 공의 실제 위치보다 골에서 유클리드로 더 멀게 만들어, 수비형이 공보다 골에서 먼 쪽에 머물며 다가오는 공을 계속 자기 골 쪽으로 떠미는 패턴을 유발함을 헤드리스 10분 시뮬레이션(득점 시점별 근접 로봇 로그)으로 확인. 목표 거리를 공의 실제 현재 위치 거리 이내로 캡. (테스트: 접선 우세 시나리오 재현 유닛테스트로 RED→GREEN, cargo test 103/103). 몸통 충돌 디플렉션에 의한 잔여 자책골은 별도 이슈(위 백로그).
