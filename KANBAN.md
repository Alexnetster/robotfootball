...
**남은 관찰/부채 (후속) 추가**
- **자살골 고빈도(조사중, KB-61 부분수정)** — 4대 헤드리스 10분 시뮬레이션 재현: 득점의 약 58%가 자책골. 원인 두 갈래: (1) `guard_target`의 LOOKAHEAD 외삽이 공 속도의 접선(골라인과 나란한) 성분이 클 때 목표를 공의 실제 위치보다 골에서 더 멀게 만들어 수비형이 공 뒤(미드필드 쪽)에서 쫓아가며 계속 떠밀던 문제 — KB-61로 수정(목표는 공의 실제 위치보다 골에서 더 멀어지지 않게 캡). (2) **미해결**: 빠르게 굴러오는 공에 로봇(공격/수비 불문)이 정면으로 돌진해 충돌 시, 반사/편향이 자기 골 쪽으로 향해 벌어지는 own-net 디플렉션 — `wants_kick`의 자책골 회피는 "차기"에만 적용되고 몸통 충돌 반사에는 적용 안 됨. 수정 전/후 10분 헤드리스 비교: KB-60 직후 17골 중 10 자책골(59%) → KB-61 이후 12골 중 7 자책골(58%, 총 득점 빈도는 줄었으나 비율은 유지). 몸통 충돌 디플렉션은 별도 설계 필요(사용자 논의 대기).
